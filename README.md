...
| Atomize | `tasks-<feature>.md` | Atomic tasks. Each names a real file, a real function signature, ≥3 verifiable acceptance criteria, and a test. Every task closes a user story (`As a ... I want ... so that ...`). |
| Validate | `revise-report.md` | Nine coverage + schema checks (C1–C9). Sets `executor_gate: pass` or `fail`. A failure regenerates the offending file once; if still failing, execution halts with a named `remaining_issues` list. |
...
missing external-accounts, missing revise-report, or a failing
`executor_gate` are present. The executor's preflight runs this same
validator and refuses to start on a red gate.
...
| `scripts/validate-instantiation.sh` | Mechanical validator. Refuses to pass on template leaks, schema violations, missing companions, or failing revise gate. |
