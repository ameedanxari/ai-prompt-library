...
export class FintechTemplateValidator extends TemplateValidator {
  ...
  validateAccountManagementRequirements(): FintechRequirements {
    const accountTemplate = join(this.moduleDir, 'account-management.md');
    const transactionTemplate = join(this.moduleDir, 'transaction-processing.md');
    const reportingTemplate = join(this.moduleDir, 'financial-reporting.md');
  ...
  validateTemplateContent(templatePath: string): BaseTemplateContent {
    ...
    return {
      ...
      // uniform across all validators (was case-sensitive here, rejecting valid
      // agent-written headings like `## purpose`).
      ...
      // over-strict: a fenced JSON/YAML example is still a code example).
