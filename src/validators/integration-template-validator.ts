...
export class IntegrationTemplateValidator extends TemplateValidator {
  constructor(modulePath: string = 'prompts/modules/integration') {
    super(modulePath);
  }
  ...
  private getEmptyTemplateContent(): IntegrationTemplateContent {
    return this.getEmptyContent<IntegrationTemplateContent>(['hasPurposeSection', 'hasContextSection', 'hasImplementationPatterns', 'hasConfigurationExamples', 'hasIntegrationPoints', 'hasSecurityConsiderations', 'hasTestingConsiderations', 'hasCodeExamples', 'hasCoreComponents']);
  }
  ...
  validateAPIManagementFeatureCoverage(): {
    ...
    const apiManagementPath = join(this.moduleDir, 'api-management.md');
    const webhookPath = join(this.moduleDir, 'webhook-systems.md');
    const messageQueuePath = join(this.moduleDir, 'message-queues.md');
    const serviceIntegrationPath = join(this.moduleDir, 'service-integration.md');
  ...
  validateEnterpriseIntegrationFeatureCoverage(): {
    ...
    const eventDrivenPath = join(this.moduleDir, 'event-driven-architecture.md');
    const dataSyncPath = join(this.moduleDir, 'data-synchronization.md');
    const enterprisePath = join(this.moduleDir, 'enterprise-integration.md');
    const monitoringPath = join(this.moduleDir, 'integration-monitoring.md');
