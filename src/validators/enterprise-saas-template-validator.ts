...
export class EnterpriseSaaSTemplateValidator extends TemplateValidator {
  constructor(modulePath: string = 'prompts/modules/enterprise-saas') {
    super(modulePath);
  }
  ...
  private getEmptyTemplateContent(): EnterpriseTemplateContent {
    return this.getEmptyContent<EnterpriseTemplateContent>(['hasPurposeSection', 'hasContextSection', 'hasImplementationPatterns', 'hasConfigurationParameters', 'hasIntegrationPoints', 'hasSecurityConsiderations', 'hasComplianceRequirements', 'hasTestingConsiderations', 'hasCodeExamples', 'hasDataModels']);
  }

  analyzeMultiTenancyCoverage(): MultiTenancyCoverage {
    const content = (this.readTemplate(this.resolveTemplate('multi-tenancy.md')) ?? '');
  ...
  analyzeRBACCoverage(): RBACCoverage {
    const content = (this.readTemplate(this.resolveTemplate('rbac-enterprise.md')) ?? '');
  ...
  analyzeSSOCoverage(): SSOCoverage {
    const content = (this.readTemplate(this.resolveTemplate('sso-integration.md')) ?? '');
  ...
  analyzeAuditTrailsCoverage(): AuditTrailsCoverage {
    const content = (this.readTemplate(this.resolveTemplate('audit-trails.md')) ?? '');
  ...
  analyzeEnterpriseBillingCoverage(): EnterpriseBillingCoverage {
    const content = (this.readTemplate(this.resolveTemplate('enterprise-billing.md')) ?? '');
  ...
  analyzeWorkflowAutomationCoverage(): WorkflowAutomationCoverage {
    const content = (this.readTemplate(this.resolveTemplate('workflow-automation.md')) ?? '');
  ...
  analyzeAPIManagementCoverage(): APIManagementCoverage {
    const content = (this.readTemplate(this.resolveTemplate('api-management.md')) ?? '');
  ...
  analyzeWhiteLabelingCoverage(): WhiteLabelingCoverage {
    const content = (this.readTemplate(this.resolveTemplate('white-labeling.md')) ?? '');
