...
export class RealTimeCommunicationTemplateValidator extends TemplateValidator {
  constructor(modulePath: string = 'prompts/modules/real-time-communication') {
    super(modulePath);
  }
  ...
  analyzeWebSocketManagementCoverage(): WebSocketManagementCoverage {
    const content = (this.readTemplate(this.resolveTemplate('websocket-management.md')) ?? '');
  ...
  analyzeMessageQueuingCoverage(): MessageQueuingCoverage {
    const content = (this.readTemplate(this.resolveTemplate('message-queuing.md')) ?? '');
  ...
  analyzePresenceSystemsCoverage(): PresenceSystemsCoverage {
    const content = (this.readTemplate(this.resolveTemplate('presence-systems.md')) ?? '');
  ...
  analyzeRealTimeSyncCoverage(): RealTimeSyncCoverage {
    const content = (this.readTemplate(this.resolveTemplate('real-time-sync.md')) ?? '');
  ...
  private getEmptyTemplateContent(): RealTimeTemplateContent {
    return this.getEmptyContent<RealTimeTemplateContent>(['hasPurposeSection', 'hasContextSection', 'hasImplementationPatterns', 'hasConfigurationParameters', 'hasIntegrationPoints', 'hasSecurityConsiderations', 'hasComplianceRequirements', 'hasTestingConsiderations', 'hasCodeExamples', 'hasDataModels']);
  }
}