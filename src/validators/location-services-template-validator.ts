...
export class LocationServicesTemplateValidator extends TemplateValidator {
  constructor(modulePath: string) {
    super(modulePath);
  }

  validateLocationServicesTemplateCompleteness(): LocationServicesTemplateStructure {
    const gpsTrackingPath = join(this.moduleDir, 'gps-tracking.md');
    const mapIntegrationPath = join(this.moduleDir, 'map-integration.md');
    const geofencingPath = join(this.moduleDir, 'geofencing.md');
    const locationPrivacyPath = join(this.moduleDir, 'location-privacy.md');
    const serviceMatchingPath = join(this.moduleDir, 'service-matching.md');
    const bookingManagementPath = join(this.moduleDir, 'booking-management.md');
    const dynamicPricingPath = join(this.moduleDir, 'dynamic-pricing.md');
    const fleetManagementPath = join(this.moduleDir, 'fleet-management.md');
  ...
  validateTemplateContent(templatePath: string): LocationServicesTemplateContent {
    ...
    return {
      ...
      // DECISION (item 12b): majority OR behaviour (was fence AND language-keyword).
  ...
  private getEmptyTemplateContent(): LocationServicesTemplateContent {
    return this.getEmptyContent<LocationServicesTemplateContent>(['hasPurposeSection', 'hasContextSection', 'hasImplementationPatterns', 'hasConfigurationParameters', 'hasIntegrationPoints', 'hasImplementationChecklist', 'hasSuccessMetrics', 'hasCodeExamples', 'hasPrivacyConsiderations', 'hasSecurityConsiderations', 'hasPlatformSpecificImplementation', 'hasPerformanceOptimization', 'hasTestingStrategy', 'hasErrorHandling']);
  }
}