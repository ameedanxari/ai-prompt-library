...
export class DeploymentTemplateValidator extends TemplateValidator {
  constructor(modulePath: string = 'prompts/modules/deployment') {
    super(modulePath);
  }
  ...
  private getEmptyTemplateContent(): DeploymentTemplateContent {
    return this.getEmptyContent<DeploymentTemplateContent>(['hasPurposeSection', 'hasContextSection', 'hasImplementationPatterns', 'hasConfigurationExamples', 'hasIntegrationPoints', 'hasSecurityConsiderations', 'hasTestingConsiderations', 'hasCodeExamples', 'hasCoreComponents']);
  }
  ...
  validateDeploymentFeatureCoverage(): {
    ...
    const containerizationPath = join(this.moduleDir, 'containerization.md');
    const kubernetesPath = join(this.moduleDir, 'kubernetes-deployment.md');
    const cloudPath = join(this.moduleDir, 'cloud-deployment.md');
    const monitoringPath = join(this.moduleDir, 'monitoring-observability.md');
    const gcpPath = join(this.moduleDir, '..', 'technology-stacks', 'cloud-gcp.md');
    const regulatedLandingZonePath = join(this.moduleDir, 'regulated-cloud-landing-zone.md');
