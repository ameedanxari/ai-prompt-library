...
export class SocialTemplateValidator extends TemplateValidator {
  constructor(modulePath: string = 'prompts/modules/social') {
    super(modulePath);
  }
  ...
  validateSocialContentEngagementFeatureCoverage(): {
    ...
    const contentFeedsPath = join(this.moduleDir, 'content-feeds.md');
    const contentCreationPath = join(this.moduleDir, 'content-creation.md');
    const engagementFeaturesPath = join(this.moduleDir, 'engagement-features.md');
    const contentModerationPath = join(this.moduleDir, 'content-moderation.md');
  ...
  validateSocialMessagingFeatureCoverage(): {
    ...
    const realTimeMessagingPath = join(this.moduleDir, 'real-time-messaging.md');
    const messageEncryptionPath = join(this.moduleDir, 'message-encryption.md');
    const voiceVideoCallsPath = join(this.moduleDir, 'voice-video-calls.md');
    const communicationModerationPath = join(this.moduleDir, 'communication-moderation.md');
  ...
  validateSocialProfileFeatureCoverage(): {
    ...
    const userProfilesPath = join(this.moduleDir, 'user-profiles.md');
    const socialGraphsPath = join(this.moduleDir, 'social-graphs.md');
    const userVerificationPath = join(this.moduleDir, 'user-verification.md');
    const socialDiscoveryPath = join(this.moduleDir, 'social-discovery.md');
  ...
  private getEmptyTemplateContent(): SocialTemplateContent {
    return this.getEmptyContent<SocialTemplateContent>(['hasPurposeSection', 'hasContextSection', 'hasImplementationGuidance', 'hasDataModels', 'hasIntegrationPatterns', 'hasSecurityConsiderations', 'hasUserExperiencePatterns', 'hasPerformanceOptimization', 'hasTestingStrategy', 'hasRealWorldConsiderations', 'hasCodeExamples']);
  }
}