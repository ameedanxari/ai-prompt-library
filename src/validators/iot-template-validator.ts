...
export class IoTTemplateValidator extends TemplateValidator {
  constructor(modulePath: string = 'prompts/modules/iot') {
    super(modulePath);
  }
  ...
  private getEmptyTemplateContent(): IoTTemplateContent {
    return this.getEmptyContent<IoTTemplateContent>(['hasPurposeSection', 'hasContextSection', 'hasImplementationPatterns', 'hasConfigurationParameters', 'hasIntegrationPoints', 'hasTestingConsiderations', 'hasSecurityConsiderations', 'hasComplianceGuidelines', 'hasCodeExamples', 'hasDataModels']);
  }
  ...
  validateIoTFeatureCoverage(): {
    ...
    const connectivityPath = join(this.moduleDir, 'device-connectivity.md');
    const managementPath = join(this.moduleDir, 'device-management.md');
    const securityPath = join(this.moduleDir, 'iot-security.md');
    const edgePath = join(this.moduleDir, 'edge-computing.md');
    const sensorPath = join(this.moduleDir, 'sensor-data-processing.md');
    const automationPath = join(this.moduleDir, 'iot-automation.md');
    const industrialPath = join(this.moduleDir, 'industrial-iot.md');
    const analyticsPath = join(this.moduleDir, 'iot-analytics.md');
