...
export class MediaStreamingTemplateValidator extends TemplateValidator {
  constructor(modulePath: string = 'prompts/modules/media-streaming') {
    super(modulePath);
  }
  ...
  validateContentDeliveryCoverage(): ContentDeliveryCoverage {
    const cdnContent = (this.readTemplate(this.resolveTemplate('cdn-integration.md')) ?? '');
    const mediaProcessingContent = (this.readTemplate(this.resolveTemplate('media-processing.md')) ?? '');
    const offlineSyncContent = (this.readTemplate(this.resolveTemplate('offline-sync.md')) ?? '');
    const streamingQualityContent = (this.readTemplate(this.resolveTemplate('streaming-quality.md')) ?? '');
  ...
  validatePlaylistDiscoveryCoverage(): PlaylistDiscoveryCoverage {
    const playlistContent = (this.readTemplate(this.resolveTemplate('playlist-management.md')) ?? '');
    const recommendationContent = (this.readTemplate(this.resolveTemplate('recommendation-engine.md')) ?? '');
    const searchContent = (this.readTemplate(this.resolveTemplate('content-search.md')) ?? '');
    const creatorContent = (this.readTemplate(this.resolveTemplate('artist-creator-tools.md')) ?? '');
  ...
  private getEmptyTemplateContent(): MediaStreamingTemplateContent {
    return this.getEmptyContent<MediaStreamingTemplateContent>(['hasPurposeSection', 'hasContextSection', 'hasImplementationPatterns', 'hasConfigurationParameters', 'hasIntegrationPoints', 'hasTestingStrategy', 'hasBestPractices', 'hasCodeExamples', 'hasPerformanceConsiderations', 'hasPlatformSpecificImplementations']);
  }
}