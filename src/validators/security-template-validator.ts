...
export class SecurityTemplateValidator extends TemplateValidator {
  constructor(modulePath: string = 'prompts/modules/security') {
    super(modulePath);
  }
  ...
  private getEmptyTemplateContent(): SecurityTemplateContent {
    return this.getEmptyContent<SecurityTemplateContent>(['hasPurposeSection', 'hasContextSection', 'hasImplementationPatterns', 'hasConfigurationParameters', 'hasIntegrationPoints', 'hasTestingConsiderations', 'hasSecurityConsiderations', 'hasComplianceGuidelines', 'hasCodeExamples', 'hasDataModels']);
  }
  ...
  validateSecurityFeatureCoverage(): {
    ...
    const mfaPath = join(this.moduleDir, 'multi-factor-auth.md');
    const authzPath = join(this.moduleDir, 'advanced-authorization.md');
    const adaptivePath = join(this.moduleDir, 'adaptive-authentication.md');
    const federationPath = join(this.moduleDir, 'identity-federation.md');
    const encryptionPath = join(this.moduleDir, 'data-encryption.md');
    const privacyPath = join(this.moduleDir, 'privacy-controls.md');
    const threatPath = join(this.moduleDir, 'threat-detection.md');
    const zeroTrustPath = join(this.moduleDir, 'zero-trust-architecture.md');
