...
export class AnalyticsTemplateValidator extends TemplateValidator {
  constructor(modulePath: string = 'prompts/modules/analytics') {
    super(modulePath);
  }
  ...
  private getEmptyTemplateContent(): AnalyticsTemplateContent {
    return this.getEmptyContent<AnalyticsTemplateContent>(['hasPurposeSection', 'hasContextSection', 'hasImplementationPatterns', 'hasConfigurationParameters', 'hasIntegrationPoints', 'hasTestingConsiderations', 'hasSecurityConsiderations', 'hasCodeExamples', 'hasDataModels']);
  }
  ...
  validateAnalyticsFeatureCoverage(): {
    ...
    const userAnalyticsPath = join(this.moduleDir, 'user-analytics.md');
    const cohortAnalysisPath = join(this.moduleDir, 'cohort-analysis.md');
    const abTestingPath = join(this.moduleDir, 'ab-testing.md');
    const privacyAnalyticsPath = join(this.moduleDir, 'privacy-analytics.md');
    const businessMetricsPath = join(this.moduleDir, 'business-metrics.md');
    const predictiveAnalyticsPath = join(this.moduleDir, 'predictive-analytics.md');
    const realTimeAnalyticsPath = join(this.moduleDir, 'real-time-analytics.md');
    const customReportingPath = join(this.moduleDir, 'custom-reporting.md');
