...
export class GamificationTemplateValidator extends TemplateValidator {
  constructor(modulePath: string = 'prompts/modules/gamification') {
    super(modulePath);
  }
  ...
  private getEmptyTemplateContent(): GamificationTemplateContent {
    return this.getEmptyContent<GamificationTemplateContent>(['hasPurposeSection', 'hasContextSection', 'hasImplementationPatterns', 'hasConfigurationParameters', 'hasIntegrationPoints', 'hasTestingConsiderations', 'hasSecurityConsiderations', 'hasCodeExamples', 'hasDataModels']);
  }
  ...
  validateGamificationFeatureCoverage(): {
    ...
    const pointSystemsPath = join(this.moduleDir, 'point-systems.md');
    const achievementSystemsPath = join(this.moduleDir, 'achievement-systems.md');
    const leaderboardsPath = join(this.moduleDir, 'leaderboards.md');
    const progressionSystemsPath = join(this.moduleDir, 'progression-systems.md');
    const socialChallengesPath = join(this.moduleDir, 'social-challenges.md');
    const rewardSystemsPath = join(this.moduleDir, 'reward-systems.md');
    const streakTrackingPath = join(this.moduleDir, 'streak-tracking.md');
    const engagementPsychologyPath = join(this.moduleDir, 'engagement-psychology.md');
