...
export class NotificationTemplateValidator extends TemplateValidator {
  constructor(modulePath: string = 'prompts/modules/notifications') {
    super(modulePath);
  }
  ...
  private getEmptyTemplateContent(): NotificationTemplateContent {
    return this.getEmptyContent<NotificationTemplateContent>(['hasPurposeSection', 'hasContextSection', 'hasImplementationPatterns', 'hasConfigurationParameters', 'hasIntegrationPoints', 'hasTestingConsiderations', 'hasSecurityConsiderations', 'hasComplianceGuidelines', 'hasCodeExamples', 'hasDataModels']);
  }
  ...
  validateNotificationFeatureCoverage(): {
    ...
    const channelsPath = join(this.moduleDir, 'notification-channels.md');
    const personalizationPath = join(this.moduleDir, 'notification-personalization.md');
    const compliancePath = join(this.moduleDir, 'notification-compliance.md');
    const analyticsPath = join(this.moduleDir, 'notification-analytics.md');
    const richPath = join(this.moduleDir, 'rich-notifications.md');
    const automationPath = join(this.moduleDir, 'communication-automation.md');
    const enterprisePath = join(this.moduleDir, 'enterprise-communications.md');
    const realTimePath = join(this.moduleDir, 'real-time-notifications.md');
