...
export class CommerceTemplateValidator extends TemplateValidator {
  constructor(modulePath: string = 'prompts/modules/commerce') {
    super(modulePath);
  }
  ...
  validateProductTemplateCompleteness(): ProductTemplateStructure {
    ...
    for (const template of productTemplates) {
      ...
      if (existsSync(templatePath)) {
        ...
        if (!this.templateHasDataModels(templatePath)) {
  ...
  private templateHasDataModels(templatePath: string): boolean {
    // File-based convenience wrapper around the shared TemplateValidator.hasDataModels.
    const content = this.readTemplate(templatePath);
    return content !== null && this.hasDataModels(content);
  }

  private getEmptyTemplateContent(): PaymentTemplateContent {
    return this.getEmptyContent<PaymentTemplateContent>(['hasPurposeSection', 'hasContextSection', 'hasImplementationPatterns', 'hasConfigurationParameters', 'hasIntegrationPoints', 'hasImplementationChecklist', 'hasSuccessMetrics', 'hasCommonPitfalls', 'hasRelatedTemplates', 'hasCodeExamples', 'hasSecurityConsiderations']);
  }
  ...
  validatePaymentProviderCoverage(): {
    ...
    const paymentProcessingPath = join(this.moduleDir, 'payment-processing.md');
    const paymentMethodsPath = join(this.moduleDir, 'payment-methods.md');
  ...
  validateShoppingWorkflowTemplates(validationMethod?: string): ShoppingWorkflowTemplateStructure {
    const shoppingCartExists = this.templateExists(this.resolveTemplate('shopping-cart.md'));
    const checkoutWorkflowExists = this.templateExists(this.resolveTemplate('checkout-workflow.md'));
    const orderManagementExists = this.templateExists(this.resolveTemplate('order-management.md'));
    const marketplaceFeaturesExists = this.templateExists(this.resolveTemplate('marketplace-features.md'));
  ...
  getShoppingWorkflowTemplates(accessPattern?: string): Record<string, any> {
    const templates = {
      cart: {
        content: (this.readTemplate(this.resolveTemplate('shopping-cart.md')) ?? ''),
        hasTypeScriptInterfaces: false,
        hasImplementationExamples: false,
        hasServiceClasses: false
      },
      checkout: {
        content: (this.readTemplate(this.resolveTemplate('checkout-workflow.md')) ?? ''),
        hasTypeScriptInterfaces: false,
        hasImplementationExamples: false,
        hasServiceClasses: false
      },
      order: {
        content: (this.readTemplate(this.resolveTemplate('order-management.md')) ?? ''),
        hasTypeScriptInterfaces: false,
        hasImplementationExamples: false,
        hasServiceClasses: false
      },
      marketplace: {
        content: (this.readTemplate(this.resolveTemplate('marketplace-features.md')) ?? ''),
        hasTypeScriptInterfaces: false,
        hasImplementationExamples: false,
        hasServiceClasses: false
      }
  ...
  analyzeShoppingWorkflowCoverage(): ShoppingWorkflowCoverage {
    const cartContent = (this.readTemplate(this.resolveTemplate('shopping-cart.md')) ?? '');
    const checkoutContent = (this.readTemplate(this.resolveTemplate('checkout-workflow.md')) ?? '');
    const orderContent = (this.readTemplate(this.resolveTemplate('order-management.md')) ?? '');
    const marketplaceContent = (this.readTemplate(this.resolveTemplate('marketplace-features.md')) ?? '');
