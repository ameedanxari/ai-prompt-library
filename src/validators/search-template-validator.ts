...
export class SearchTemplateValidator extends TemplateValidator {
  constructor(modulePath: string = 'prompts/modules/search-discovery') {
    super(modulePath);
  }
  ...
  private getEmptyTemplateContent(): SearchTemplateContent {
    return this.getEmptyContent<SearchTemplateContent>(['hasPurposeSection', 'hasContextSection', 'hasImplementationPatterns', 'hasConfigurationParameters', 'hasIntegrationPoints', 'hasTestingConsiderations', 'hasSecurityConsiderations', 'hasCodeExamples', 'hasDataModels']);
  }
  ...
  validateSearchFeatureCoverage(): {
    ...
    const fullTextPath = join(this.moduleDir, 'full-text-search.md');
    const facetedPath = join(this.moduleDir, 'faceted-search.md');
    const analyticsPath = join(this.moduleDir, 'search-analytics.md');
    const personalizationPath = join(this.moduleDir, 'search-personalization.md');
